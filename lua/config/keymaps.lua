...
local function get_root()
  return vim.fs.root(0, { ".git", "package.json", "Makefile", "Cargo.toml" }) or vim.uv.cwd()
end
map("n", "<leader>fT", function() Snacks.terminal() end, { desc = "Terminal (cwd)" })
map("n", "<leader>ft", function() Snacks.terminal(nil, { cwd = get_root() }) end, { desc = "Terminal (Root Dir)" })
map({"n","t"}, "<c-/>",function() Snacks.terminal(nil, { cwd = get_root() }) end, { desc = "Terminal (Root Dir)" })
map({"n","t"}, "<c-_>",function() Snacks.terminal(nil, { cwd = get_root() }) end, { desc = "which_key_ignore" })
